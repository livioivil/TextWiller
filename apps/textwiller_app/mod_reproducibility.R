...
mod_reproducibility_ui <- function(id) {
  ...
  shiny::wellPanel(
    ...
    shiny::p("Export or reload the analysis history to ensure full reproducibility."),
    shiny::fluidRow(
      shiny::column(6,
        shiny::actionButton(ns("refresh_btn"), "Refresh", icon = shiny::icon("sync"), class = "btn-primary btn-sm btn-block")
      ),
      shiny::column(6,
        shiny::actionButton(ns("clear_btn"), "Clear history", icon = shiny::icon("trash"), class = "btn-warning btn-sm btn-block")
      )
    ),
    shiny::br(),
    shiny::tableOutput(ns("history_table")),
    shiny::br(),
    shiny::downloadButton(ns("download_script"), "Download R script", class = "btn-success btn-sm"),
    shiny::downloadButton(ns("download_history"), "Download history (.rds)", class = "btn-default btn-sm"),
    shiny::br(), shiny::br(),
    shiny::fileInput(ns("upload_history"), "Upload history (.rds)", accept = ".rds"),
    shiny::checkboxInput(ns("append_history"), "Append to existing history", value = TRUE),
...
mod_reproducibility_server <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$refresh_btn, {
      refresh_summary()
      shiny::showNotification("History refreshed", type = "message")
    })
    
    shiny::observeEvent(input$clear_btn, {
      history$clear()
      refresh_summary()
      shiny::showNotification("History cleared", type = "warning")
    })
    ...
    shiny::observeEvent(input$upload_history, {
      ...
      tryCatch({
        ...
        refresh_summary()
        shiny::showNotification("History imported successfully", type = "message")
      }, error = function(e) {
        shiny::showNotification(paste("Error loading history:", e$message), type = "error")
      })
    })
    ...
    output$history_table <- shiny::renderTable({
      ...
      if (is.null(data) || nrow(data) == 0) {
        return(data.frame(Message = "No operations recorded yet"))
      }
      data
